...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
        ...
        # for wise-iou loss
        self.use_wiseiou = False   # 原始的语句是这样的，这里我使用WIOU系列的，因此改为下面语句
        #self.use_wiseiou = True
        if self.use_wiseiou:
            #self.wiou_loss = WiseIouLoss(ltype='WIoU', monotonous=False, inner_iou=False, focaler_iou=False)   # 原始语句是这样的，但是我想要使用Ineer-ShapeIOU，因此改为下面那种形式
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if self.use_wiseiou:
            # loss[name_giou] = self.wiou_loss(pred_bboxes, gt_bboxes, ret_iou=False, ratio=0.7, d=0.0, u=0.95) 原本的语句是这个，但是我改成了Ineer-ShapeIOU，因此改成了下面这种形式
            ...
            # loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=True)  # 原本的语句是这个，但是我想还用powerIOU，因此改成了下面那种
            # loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, PIOU=True)
            loss[name_giou] = 1.0 - bbox_inner_iou(pred_bboxes, gt_bboxes, xywh=True, PIoU=True, ratio=0.7) # Inner IoU
